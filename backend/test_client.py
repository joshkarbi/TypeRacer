...
async def hello():
    ...
    async with websockets.connect(uri) as websocket:
        ...
        print(game_response)
        join_game = {"type": "join_game", "game_ID": game_response["game_ID"]}
        await websocket.send(json.dumps(join_game))
        
        response = json.loads(await websocket.recv())
        print(response)
        PLAYER_ID = response["player_IDs"][-1]
        player_ready = {"type":"player_status", "status": "ready", "game_ID": game_response["game_ID"], "player_ID": response["player_IDs"][-1]}
        ...
        for word in range(len(game_response["paragraph"].split(" "))):
            typed = {"type": "update", "game_ID": game_response["game_ID"], "player_ID": PLAYER_ID, "word_num": word+1}
